...
async function sendDueNotifications() {
  ...
  // Reads whatever columns exist (missed_this_cycle is applied lazily by
  // routes/debt.js, and the cron can fire before the first API request) --
  // a missing column just means nothing has been marked missed yet.
  const missedIds = cashflowResult.rows[0]?.missed_this_cycle || [];
  const today = new Date().getDate();
  const daysAhead = settings.notify_days_before || 3;

  for (const debt of debtsResult.rows) {
    if (debt.archived) continue; // hidden from the plan — no reminders either
    if (!debt.due || Number(debt.min) <= 0) continue;
    if (paidIds.includes(debt.id) || missedIds.includes(debt.id)) continue;
...
// Nudges once a cycle has been open 28+ days, so anything still unticked gets
// seen before the automatic rollover records it as an unmet floor
// (lib/debtCycle.js). Mirrors GET /debt/api/cycle-status, which the frontend
// polls for the in-app banner version of the same check.
async function checkCycleReset() {
  ...
  await broadcast(
    `Your cycle is ${daysSinceStart} days old and renews shortly — any floor still unmet goes onto the catch-up ledger.`,
    { title: 'Debt Plan — cycle renews soon', priority: 'default', tags: 'recycle', tag: 'debt-cycle-reset' }
  );
}
