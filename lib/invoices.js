...
function interimInvoiceMath(p) {
  ...
  if (!errors.length && !(amountDue > 0)) {
    errors.push(subtotal > 0
      ...
      : 'Nothing to bill — raise a percentage, mark a room done, or tick materials.');
  }
  return {
...
function interimInvoiceLineItems(p) {
  ...
  var lines = [];
  (p.labour || []).concat(p.variations || []).forEach(function(l) {
    if (!(Math.abs(+l.amount || 0) > 0.005)) return;
...
function planInterimInvoice({ existing, depositTotal, body }) {
  ...
  return {
    row: {
      ...
      lineItems: interimInvoiceLineItems({ labour: labourLines, variations: variationLines, materials }),
      idempotencyKey: key
    }
  };
}
