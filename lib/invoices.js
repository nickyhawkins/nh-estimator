...
function interimInvoiceMath(p) {
  ...
  // The trim is spread over this invoice's billing labour lines in proportion
  // (the quote's own rounding spread), the remainder landing on the largest.
  // No trim when there's no labour to take it from, or when it would leave
  // nothing to bill at all.
  var step = 5;
  var target = Math.floor(Math.round(subtotal * 100) / (step * 100)) * step;
  var trim = money(subtotal - target);
  var roundingTrim = 0;
  if (trim > 0.005 && target > 0 && labourAmount >= trim) {
    var largest = null, trimmed = 0;
    labourLines.forEach(function(l) {
      if (!(l.amount > 0)) return;
      var cut = money(trim * l.amount / labourAmount);
