...
// verdict, same floor-shortfall ledger, same things left alone -- pots and
// buffer carry over, floor_shortfalls accumulates rather than resetting.
// It is the button pressing itself, not a second, different policy. In
// particular it does NOT add unmet floors to arrears: the floor-shortfall
// ledger is where a missed floor is recorded, deliberately inert until the
// user taps Catch up (see the note in getCycleArchivePayload).
//
...
// applied_payments (what each tick actually took off a balance).
...
async function rollOnce() {
  ...
  try {
    ...
    for (const d of inPlan) {
      ...
      if (floor === null) continue; // no floor agreed -- out of the verdict
      const due = Math.min(floor, Number(d.balance));
      withFloor.push(d);
      if (paidIds.includes(d.id) || floorPaidIds.includes(d.id)) continue;
      unmet.push({ id: d.id, name: d.name, amount: round2(due) });
    }
    const floorsMet = withFloor.length > 0 && unmet.length === 0;
    // Stricter, as on the client: every debt in the plan paid in full.
    const targetMet = inPlan.length > 0 && inPlan.every(d => paidIds.includes(d.id));
    ...
    return { cycleNumber, closedAt: dueAt, totalPaid, floorsMet, unmet };
  } catch (err) {
...
async function notifyRolled(rolled) {
  for (const r of rolled) {
    const n = r.unmet.length;
    const body = r.floorsMet
      ? `Cycle ${r.cycleNumber} closed with every floor met. Open the app to sync your balances.`
      : n
        ? `Cycle ${r.cycleNumber} closed. ${n} floor${n === 1 ? '' : 's'} went unmet and ${n === 1 ? 'is' : 'are'} on the catch-up ledger — open the app to sync your balances.`
        : `Cycle ${r.cycleNumber} closed. Open the app to sync your balances.`;
    await broadcast(body, {
      title: 'Debt Plan — new cycle started',
      priority: r.floorsMet ? 'default' : 'high',
      tags: 'recycle',
