...
async function rollOnce() {
  ...
  try {
    ...
    for (const d of inPlan) {
      ...
      if (floor !== null) {
        withFloor.push(d);
        if (!paidIds.includes(d.id) && !floorPaidIds.includes(d.id)) {
          unmet.push({ id: d.id, name: d.name, amount: round2(floorDue) });
        }
      }
      ...
      const coveredByPayment = paidIds.includes(d.id) ? minDue
