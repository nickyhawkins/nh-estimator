...
router.get('/exterior', async (req, res) => {
  try {
    const result = await db.query('SELECT data FROM exterior_state WHERE id = 1');
    res.json(result.rows[0]?.data || {});
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.put('/exterior', async (req, res) => {
  try {
    await db.query(
      'UPDATE exterior_state SET data = $1, updated_at = NOW() WHERE id = 1',
      [req.body]
    );
    res.json({ ok: true });
  } catch (err) {
...
router.delete('/exterior', async (req, res) => {
  try {
    await db.query("UPDATE exterior_state SET data = '{}', updated_at = NOW() WHERE id = 1");
    res.json({ ok: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.delete('/all', async (req, res) => {
  try {
    ...
    await db.query("UPDATE exterior_state SET data = '{}' WHERE id = 1");
    res.json({ ok: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
