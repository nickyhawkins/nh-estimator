...
- **314 is NOT the cost side of the sundries %.** The % (labour × %) covers stock consumables used across jobs — caulk, tape, filler, floor protection — and stays a percentage, never itemised, never tracked. 314 is for job-specific consumables the % won't cover (wallpaper paste, lining paper), which are real Xero items tracked like paint. Itemising something the % already covers charges the client twice.
