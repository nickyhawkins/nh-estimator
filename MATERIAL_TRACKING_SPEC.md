...
**~~DEPENDENCY: the app ignores `Status`, so pruning won't take effect.~~ NOT A DEPENDENCY — the prune was done by deletion, which works.** `/material-groups` filters on `SalesDetails.AccountCode` only and never checks whether an item is archived. That remains true, but it stopped blocking anything once `RPC*` and the stock `SUN` consumables were **deleted** outright rather than archived.

- **What's left of it is a small standalone bug, not a dependency.** Re-measured 2026-07-14 against the export: **exactly one** archived item is still offered as a live option — `TIK015` (`Tikkurila Anti Reflex 2 - Magnolia 3ltr`). (The earlier note also named `SUN016 Tesa Easy Cover`; that code now holds `F&B Wallpaper Paste`, so the claim rotted with the data.) One stale row in a picker of ~1,576. **Tracked in FEATURES.md as its own loose end** — it is not a material-tracking problem.
- **Still unverified, and only matters if the fix is attempted:** the CSV export has a `Status` column, but whether the **Items API** exposes it is unconfirmed — the code doesn't read it, and this was only ever checked against an export, not the live payload. **Check the raw `/Items` JSON before designing around it.** ~~Settle this before Phase 1~~ — Phases 0, 1 and 2(a) all shipped without it, which is the proof it was never a prerequisite.
...
>
> **Addendum (2026-07-24): colour on manually-added rows, plus edit.** `material_actuals` gained `colour_number` (another `db/setup.sql` column that needs the manual `psql -f` step — same caveat as above). The "Add material the estimate missed" form now has a Colour `<select>` (same options/markup as Summary's `add-mat-colour`), and rows created from it (or picked from search but not already on the estimate — i.e. `onEstimate: false`) show their colour as sub-detail, the same way an estimate-pulled row shows its per-colour parts. A pencil icon on those rows opens an inline edit form (description/colour/price; quantity and bought stay on the main card, edited separately). **Deliberately scoped to `onEstimate: false` rows only** — an estimate-pulled row is one product that can legitimately cover more than one colour (see the reconciliation-view comment above), so a single `colourNumber` on it would misrepresent that; it keeps its existing auto-derived, read-only breakdown.
