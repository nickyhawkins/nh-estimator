...
## 15. Excluded walls — price a room minus the walls that aren't being painted — BUILT
- Need: some rooms have a wall (or walls) that stay untouched — e.g. wallpaper that's staying up — so the quote should only cover the remaining walls.
- Built: a new collapsible "Excluded Walls" card on the room form (under Dimensions), same add/remove row list as Panelling but width × height only (no coats — the wall isn't painted). Height pre-fills from the room height since an excluded wall is nearly always full-height. A running "Excluded from wall area" total shows the m² being taken off.
- Each row's width × height is subtracted from the room's wall area in `calcRoom()` before anything downstream reads it, so paint labour, wall litres, the mist coat and the feature-wall carve-out all shrink together. The live preview's "Walls" figure shows the reduced area.
- Deliberately NOT applied to room wallpaper hanging (that prices by perimeter-derived rolls, and an excluded-from-paint wall may be the very one being papered) or to the door/perimeter-based woodwork formula (skirting on an excluded wall usually still gets painted; if not, the woodwork coats control covers it).
- Not available on staircase/HSL rooms — the staircase preview knows nothing about exclusions, so the card hides in staircase mode and any leftover rows are dropped on save to stop the saved total drifting from the preview. HSL rooms already have `wallAreaOverride` for hand-tuning.
- Stored as `excludedWalls: [{id,width,height}]` on the room JSONB — no migration needed; old rooms without the field price exactly as before.

## 16. Panelling walls optionally deduct from the room wall area — BUILT
- Need: a panelled wall that's part of the room's measured walls was priced twice — once as ordinary wall paint (the room's wall area never shrank), again as panelling. The workaround was entering the same wall a second time under Excluded Walls. Prompted by a real quote: a wall that's half panelling, half regular wall, two colours, two paints.
- Built: each panelling wall row gets a "−m²" chip (on by default for newly added walls). When on, that wall's width × height is subtracted from the room's wall area in `calcRoom()`, at the same point as Excluded Walls — so paint labour, wall litres, mist coat and the feature-wall carve-out all shrink together — while `calcPanel()` still prices the panelling itself with its own coats/prep/colour/product.
- The half-panelled two-colour case: enter just the panelled portion's W×H (e.g. 4 × 1.2 wainscoting). That area leaves the wall paint calc; the emulsioned upper half stays in the wall area under the room's own colour and paint. Nothing else to do.
- Chip off = old behaviour, for panelling that isn't on a measured room wall (bath panel, boxing-in, under-stairs). A "Deducted from wall area" running total shows under the list when any chip is on, mirroring the Excluded Walls total.
- Staircase/HSL rooms: initially the flag was ignored there (the same drift rule that drops excluded walls on save — the staircase preview knew nothing about the deduction), but since panelling up a stair wall is the most common panelling case, v2.7.1 built the deduction into the staircase pipeline properly instead: `calcHSLNow()` (preview) and `computeHSLOverrides()` (save) both subtract the flagged walls' area from the geometry-derived wall total via one shared helper (`panelDeductAreaForm()`), so the two can never disagree and the chip now shows in staircase mode too. The saved `wallAreaOverride`/`wallCostStore` are net of the deduction, which is why `calcRoom()` still skips its own deduction for HSL rooms — deducting there as well would double it. Wallpaper rolls stay untouched, same as normal rooms.
- Stored as an optional `deduct` boolean on the existing `panelItems` rows, saved as absent when off — old rooms and deduct-off rooms are byte-identical in the JSONB, so existing quotes price exactly as before. No migration, no DB changes.
