...
- Staircase/HSL rooms ignore the flag (calcHSLNow()'s preview knows nothing about the deduction — the same drift rule that drops excluded walls on save there), and the chip hides in staircase mode. Unlike excluded walls, the panelling rows themselves are kept in staircase mode, so under-stairs wainscoting still prices.
