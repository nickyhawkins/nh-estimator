...
**For build v2.39.6** (27 August 2026). The build number you're running is shown at the bottom of the ☰ menu — if it doesn't match, the deploy hasn't landed yet.
...
- **Door/frame prep in bulk edit, and staircase rooms take more of it (v2.39.6).** Doors and frames have their own prep level on the room form, separate from the room's — that's now in the bulk panel too, next to the room's own, and it's skipped on any room with no doors or frames. Panelling prep isn't there: it belongs to the individual panelling rows rather than the room. **Staircase rooms also get more out of bulk edit than they did.** The last version skipped coats, mist coat *and* both prep levels on them, which was more cautious than it needed to be: doors and frames are priced separately from the staircase total, so **door coats, frame coats and door/frame prep do work on a staircase room** and are no longer refused. What still gets skipped there is what genuinely can't work — wall, ceiling and woodwork coats, the mist coat, and the room's own prep level, all of which are baked into the staircase price when you save that form. Change those on the staircase form and it recalculates properly.
- **Bulk edit now covers every paint and the prep level (v2.39.5).** The bulk panel started with wall paint only; it now has a product picker for **all seven** — walls, ceiling, woodwork topcoat, woodwork primer (with **None**, for a self-priming topcoat), mist coat, feature wall and panelling — plus the room's **prep level** (Minimal / Standard / Heavy / a custom %). Same rules as before: everything starts on **No change**, and a room that hasn't got the surface is skipped and marked N/A rather than given one. Three of those skips are worth knowing: a **mist coat paint** only reaches rooms that already have a mist coat *or are getting one from the same run* (so ticking the mist toggle and picking the mist paint together does what you'd expect); a **feature wall paint** skips a feature wall you've set to wallpaper, since no paint is bought for it; and **panelling paint** needs the room to actually have panelling rows. **Staircase / HSL rooms now behave properly too.** Their price is worked out and stored when you save the staircase form, so bulk edit changing their coats, mist coat or prep would have moved the paint quantities without moving the price — it did that in the first version, and it no longer does. Bulk edit now skips those three fields on a staircase room and says so in the preview; change them on the staircase form instead. Paint products still apply to staircase rooms normally, because those only affect quantities. Panelling prep and door/frame prep aren't in the bulk panel — they're separate settings on the room form and rarely a whole-job decision.
- **Bulk edit rooms (v2.39.2).** Changing the wall paint across a whole house, or adding a mist coat and an extra coat everywhere, no longer means opening every room in turn. **Bulk edit** on the Interior header on Measure puts a tick box on each room; pick the ones you want (or **Select all**) and a panel opens with the handful of fields worth doing in one go — wall paint product and colour band, coats for walls, ceiling, woodwork, doors and frames, and a mist coat for walls or ceiling. **Every field starts on "No change" and stays there unless you set it** — a field you don't touch is not touched on any room, and nothing carries over from the last time you used it. **Preview changes** then shows you every room you ticked and exactly what would move, `old → new`, line by line; nothing is written until you tap Apply, and Back leaves everything as it was. Two things it deliberately won't do: it **won't invent a surface a room hasn't got** (ask for three coats of woodwork and a room with no woodwork in it is skipped, marked N/A in the preview, not given some), and **mist coat only ever adds** — ticking Walls turns it on where it's off, and leaving it unticked never turns off a mist coat a room already has. Coats work the other way and the form says so: the number you type is the coat count, not an increase. Afterwards the rooms are ordinary rooms — open any one and edit it as normal, nothing is locked.
